...
const Select = ({ 
  ...
  // Only run this effect when options change
  useEffect(() => {
    log.debug(`Select options for ${listName || 'direct'}:`, {
      count: options.length,
      first: options[0] ? JSON.stringify(options[0]) : 'none',
      currentValue: value,
      renderCount: ++renderCountRef.current
    });
    
    if (listName && options.length === 0) {
      log.error(`No options found for list: ${listName}`);
    }
  }, [options, listName, value]); // Fewer dependencies
  ...
  const renderInput = useCallback((params) => (
    ...
    />
  ), [placeholder, sx, listName, optionsLength]);
