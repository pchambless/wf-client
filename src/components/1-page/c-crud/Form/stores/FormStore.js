...
class FormStore {
  ...
  constructor(config, initialData = {}) {
    ...
    // Initialize pageMap BEFORE making observable
    if (config?.pageMap) {
      verifyPageMap(config.pageMap, 'FormStore.constructor.before');
      this.pageMap = config.pageMap; // Use direct reference
      verifyPageMap(this.pageMap, 'FormStore.constructor.after');
    }
