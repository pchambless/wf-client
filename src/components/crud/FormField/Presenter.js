...
export class FormFieldPresenter {
  ...
  processColumnMap(columnMap) {
    if (!columnMap || !columnMap.columns) {
      log.error('Invalid columnMap structure', columnMap);
      return [];
    }
    
    // First filter for visible fields (group >= 0)
    const fields = columnMap.columns
      .filter(col => !col.hideInForm && (col.group === undefined || col.group >= 0))
      .map(col => this.processColumnDefinition(col))
      // Sort by group for proper rendering order
      .sort((a, b) => {
        // Sort by group first (undefined groups go last)
